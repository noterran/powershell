<#PSScriptInfo
	...
	.VERSION 3.3.4
	.AUTHOR Adopted by Marcello Team. 
	.COPYRIGHT Original 2020 CyberMoloch. All Rights Reserved.
	.LICENSE  GNU GENERAL PUBLIC LICENSE
	.PROJECTURI https://www.marcello.no
...
.NOTES
	Env:Action - Choose to install or uninstall an application on your device.
	Env:DisplayGuiToUser - Override interactive session during installation. If unchecked, the installation will be silent.
...
If (-not (Test-Path -Path Env:\CS_ONLINETASK) ) {
    if (Test-Path -Path registry::HKLM:\SOFTWARE\CentraStage\Env) {
      ...
      Write-Output ('| Workaround for User Task not set')
    }
}
...
# By default, PSADT logfiles are saved to $envWinDir\Logs\Software which doesn't exist here
...
$psadtTempPath = '${env:PUBLIC}'
$psadtLogPath = '${Env:WINDIR}\Logs\marcello-RMM'
$psadtLogPathNoAdmin = '${Env:ProgramData}\Logs\marcello-RMM'
$msiLogPath = '${Env:WINDIR}\Logs\marcello-RMM'
$msiLogPathNoAdmin = '${Env:ProgramData}\Logs\marcello-RMM'
$msiInstallParams = 'ADDLOCAL=All ALLUSERS=1 REBOOT=ReallySuppress /QB-!'
$msiSilentParams = 'ADDLOCAL=All ALLUSERS=1 REBOOT=ReallySuppress /QN'
...
If (Test-Path -Path ($psadtPath + '\AppDeployToolkitConfig.xml')) {
    ...
    If ($psadtTempPath) {
        Write-Output ('| Push temp file location: ' + $psadtTempPath)
        $psadtConfigXml.AppDeployToolkit_Config.Toolkit_Options.Toolkit_TempPath = $psadtTempPath
    } 
	If ($psadtLogPath) {
        Write-Output ('| Push Logfile location: ' + $psadtLogPath)
        $psadtConfigXml.AppDeployToolkit_Config.Toolkit_Options.Toolkit_LogPath = $psadtLogPath
    }   
    If ($psadtLogPathNoAdmin) {
        Write-Output ('| Push NoAdmin Logfile location: ' + $psadtLogPathNoAdmin)
        $psadtConfigXml.AppDeployToolkit_Config.Toolkit_Options.Toolkit_LogPathNoAdminRights = $psadtLogPathNoAdmin
    }
    If ($msiLogPath) {
        Write-Output ('| Pushing MSI Logfile location: ' + $msiLogPath)
        $psadtConfigXml.AppDeployToolkit_Config.MSI_Options.MSI_LogPath = $msiLogPath
    }
	If ($msiLogPathNoAdmin) {
        Write-Output ('| Push NoAdmin MSI Logfile location: ' + $msiLogPathNoAdmin)
        $psadtConfigXml.AppDeployToolkit_Config.MSI_Options.MSI_LogPathNoAdminRights = $msiLogPathNoAdmin
    }
	If ($msiInstallParams) {
        Write-Output ('| Push Install param: ' + $msiInstallParams)
        $psadtConfigXml.AppDeployToolkit_Config.MSI_Options.MSI_InstallParams = $msiInstallParams
    }
	If ($msiSilentParams) {
        Write-Output ('| Push Silent Install param: ' + $msiSilentParams)
        $psadtConfigXml.AppDeployToolkit_Config.MSI_Options.MSI_SilentParams = $msiSilentParams
    }
...
$psadtTempPath = $ExecutionContext.InvokeCommand.ExpandString($psadtConfigXml.AppDeployToolkit_Config.Toolkit_Options.Toolkit_TempPath)
Write-Output ('| PASADT temp file location: ' + $psadtTempPath)
...
Write-Output ('| PASADT Logfile location: ' + $psadtLogPath)
...
Write-Output ('| PASADT NoAdmin Logfile location: ' + $psadtLogPathNoAdmin)
$msiLogPath = $ExecutionContext.InvokeCommand.ExpandString($psadtConfigXml.AppDeployToolkit_Config.MSI_Options.MSI_LogPath)
Write-Output ('| MSI Logfile location: ' + $msiLogPath)
$msiLogPathNoAdmin = $ExecutionContext.InvokeCommand.ExpandString($psadtConfigXml.AppDeployToolkit_Config.MSI_Options.MSI_LogPathNoAdminRights)
Write-Output ('| PASADT NoAdmin MSI Logfile location: ' + $msiLogPathNoAdmin)
$msiInstallParams = $ExecutionContext.InvokeCommand.ExpandString($psadtConfigXml.AppDeployToolkit_Config.MSI_Options.MSI_InstallParams)
Write-Output ('| PASADT MSI Install param for Interactive install: ' + $msiInstallParams)
$msiSilentParams = $ExecutionContext.InvokeCommand.ExpandString($psadtConfigXml.AppDeployToolkit_Config.MSI_Options.MSI_SilentParams)
Write-Output ('| PASADT MSI Install param for default install: ' + $msiSilentParams)
...
if ((${env:\DisplayGuiToUser}) -eq "false"){
    $psadtMode = '-DeployMode \"NonInteractive\"'
	Write-Output ('| Display GUI:'+ ${env:\DisplayGuiToUser} + '. Running silent installation.')
}   
else {                                                          
    if ((Get-WmiObject -Class win32_computersystem).UserName) {
        Write-Output ('| Display GUI:'+ ${env:\DisplayGuiToUser} + '. User session found. Runing interactive installation.')
        ...
        Write-Output ('| Display GUI:'+ ${env:\DisplayGuiToUser} + '. User session not found. Running non-interactive installation.')
        $psadtMode = '-DeployMode \"NonInteractive\"'
...
If (Test-Path ($psadtLogPath)) {
    ...
    Write-Error ('! Unable to retrieve log directory.')
}
...
If ( ($psadtLogPathNoAdmin -ne $psadtLogPath) -and ($psadtLogPathNoAdmin -ne $msiLogPath) ) {
    If (Test-Path -Path $psadtLogPathNoAdmin) {
        ...
        Write-Error ('! Unable to retrieve NoAdmin log directory.') -ErrorAction silentlycontinue
    }
}
...
If ( ($msiLogPath -ne $psadtLogPath) -and ($msiLogPath -ne $psadtLogPathNoAdmin) ) {
    If (Test-Path ($msiLogPath)) {
        ...
        Write-Error ('! Unable to retrieve MSI log directory.')
    }
}

If ($psadtLogFiles) {
    $psadtLogFiles | ForEach-Object -Process { Write-Output ('| Logfile found: ' + $PSItem.Name) }
    $psadtLogFiles | ForEach-Object -Process {
        If (Test-Path ($PSItem.FullName)) {
            Write-Output ('')
            Write-Output ('|---------------------------(LOGFILE OUTPUT)----------------------------------|')
